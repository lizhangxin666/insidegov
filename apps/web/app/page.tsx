...
export default function Home() {
  ...
  const [detail, setDetail] = useState<"negotiations" | "unified" | "trace" | "audit" | "counterfactual" | "interview" | "material" | "about" | null>(null);
...
function AuditCard({ item }: { item: AgentActionAudit }) { return <article><strong>Q{item.quarter} · {item.agent_id} · {item.action_type}</strong><span>{item.provider}{item.fallback ? " · 已降级" : " · 原生执行"}</span><p><b>当时观察：</b>{brief(item.observation)}</p><p><b>私有信息：</b>{"redacted" in item.private_context_used ? `默认脱敏；使用字段 ${(item.private_context_used.fields_used ?? []).join("、")}` : "已记录"}</p><p><b>模型/策略建议：</b>{brief(item.llm_suggestion)}</p><p><b>规则修正：</b>{brief(item.rule_adjustment)}</p><p><b>最终执行：</b>{brief(item.executed_action)}</p><p><b>理由：</b>{item.rationale}</p><p><b>复盘：</b>{item.reflection}</p><small>诊断记录 {item.diagnostics.length} 条 · {item.outcome}</small></article>; }
