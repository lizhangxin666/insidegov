...
状态说明：**已实现**表示 v0.2 可用；**部分实现**表示已有底层能力但体验未闭环；**规划**表示目标需求。
...
| 报告 | 基础实验数据导出 | 部分实现 |
| 材料导入 | 报告/政策草案转世界配置 | 规划 |
| Agent 访谈 | 运行后追问个体认知与反事实理由 | 规划 |

### 6.2 v0.3 目标范围

v0.3 的产品主题是：**从可运行模拟器升级为完整政策实验工作台**。

- 世界构建向导：上传/粘贴材料、实体与关系抽取、参数建议、人工确认；
- 实验设计器：选择自变量、对照组、种子数、运行周期、指标和机制开关；
- 批量实验：多种子、多参数组合排队运行并聚合结果；
- 对比报告：自动生成差异、关键事件、因果链、敏感性与限制说明；
- Agent 访谈：基于该 Agent 当时可见信息和记忆回答，不泄漏世界真值；
...
| WLD-06 | 材料构建世界 | 抽取结果经人工确认后生成配置和来源记录 | 规划 |
...
| SIM-06 | 批量运行 | 可配置参数网格、种子数和并发队列 | 规划 |
...
| AGT-07 | 历史访谈 | 回答绑定当时观察和记忆，区分记录与生成解释 | 规划 |
...
| EXP-06 | 敏感性分析 | 参数范围、多种子与不确定性聚合 | 规划 |
| EXP-07 | 自动报告 | 输出差异、关键链条、限制和复现信息 | 规划 |
...
| UI-06 | 实验设计器 | 图形化定义对照、参数、种子、指标与停止条件 | 规划 |
| UI-07 | 报告中心 | 浏览、比较、引用和导出实验报告 | 规划 |
...
**0:00—0:40，问题。** 招商政策不是一句“给补贴”，它会穿过财政审核、企业选择、承诺兑现和产业扩张。纯聊天 Agent 无法保证这些约束一致。

**0:40—1:20，世界。** 展示三座城市、企业异质性、市领导与财政部门的私有信息，强调没有角色看到全部真相。

**1:20—2:10，第一次决策。** 推进招商阶段，打开政策提案—财政否决/限制—协调—企业选址轨迹，展示 LLM 负责策略、规则负责结算。

**2:10—3:10，反事实。** 从同一时点创建分支，一边保持基线，另一边施加财政或需求冲击，连续推进。

**3:10—4:10，解释差异。** 对比预算、履约率、信用、就业和产能利用率，从曲线跳转到关键违约、信用更新和供应商决策。

**4:10—5:00，研究可信度。** 展示种子、机制消融、校准任务和完整导出；最后明确产品用于政策实验和机制探索，不是未来预测器。
